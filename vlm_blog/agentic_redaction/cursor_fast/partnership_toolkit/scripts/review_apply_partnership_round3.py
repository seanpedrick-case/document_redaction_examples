...
REVIEW_CSV = Path("output/partnership_toolkit/review_cycle/partnership_review_file_round3.csv").resolve()
OUT_DIR = Path("output/partnership_toolkit/after_apply_003").resolve()
...
def main():
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    client = Client(BASE_URL, httpx_kwargs={"timeout": httpx.Timeout(connect=120.0, read=1800.0, write=120.0, pool=120.0)})
    ...
    (OUT_DIR / "apply_metadata.json").write_text(json.dumps({"server_paths": server_paths}, indent=2), encoding="utf-8")
